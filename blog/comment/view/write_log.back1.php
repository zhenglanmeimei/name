...
<article class="blogs">
  ...
  <div class="index_about">
    ...
    <p><span>关键字词</span>：爱情,犯错,原谅,分手</p>
      <div id="replay">
       <legend>发布评论<a href="#replay" id="noreplay" style="float: right;display: none;cursor:pointer;">取消回复</a></legend>
...
<form class="am-form" role="form" method="post" action="index.php?action=addcom" >
     ...
     <label>用户昵称<span class="required"></span></label>
        <input maxlength="200" style="width:232px;" class="form-control" name="nickname"required="required" />
    </li>
    <li>
    <label>邮箱地址</label>
        <input  type="email" maxlength="200" style="width:232px;" class="form-control" name="email"  required="required"/>
    </li>
    <li>
    <label>友情链接<span class="required"></span></label>
        <input type="url" maxlength="200" style="width:232px;" class="form-control" name="address"required="required"/>
    </li>
    <span><?php echo date('Y-m-d H:i:s', time()); ?></span>
    <li>描述</li>
    <li><textarea name="description" type="text" class="form-control" style="width:230px;height:200px;overflow:auto;" required="required"></textarea></li>
    <li><input type="submit" class="btn btn-primary" name="submit" value="提交评论"/></li>
</div>
</form>
      ...
      $(function() {
       // 给所有的回复按钮绑定点击事件
        $('.replay').click(function() {
            ...
            // console.log($('form input[name=pid]'));
            //把from表单移动到对应的评论下边
            // console.log($(this).attr('data-id'));
            // console.log($(this).parents('article').first().append($('#replay')));
